...
const tailwindVersion = ["3.1.8", "3.1.6"];
...
const exportTailwindCSS = () => {
  tailwindImports.forEach((file) => {
    ...
    execSync(
      `npx tailwindcss -i ./${file}.css -o ${tailwindPublicPath}/${file}.css`
    );
    execSync(
      `npx tailwindcss -i ./${file}.css -o ${tailwindPublicLatestPath}/${file}.css`
    );
  });
};
exportTailwindCSS();
...
try {
  ...
  fs.writeFileSync(
    `${tailwindPublicPath}/tailwind-utilities.less`,
    dataToLESS,
    { encoding: "utf8", flag: "w" }
  );
  fs.writeFileSync(
    `${tailwindPublicLatestPath}/tailwind-utilities.less`,
    dataToLESS,
    { encoding: "utf8", flag: "w" }
  );
} catch (err) {
